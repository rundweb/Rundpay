    ...
    <div className="flex flex-col p-10 max-w-7xl m-auto items-center gap-10 bg-white rounded-2xl lg:px-20 lg:py-12">
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div className="flex flex-col gap-1">
          <p className="text-blue-primary text-sm font-semibold tracking-wide">FUTURE PAYMENT</p>
          <h1 className="text-black-secondary text-3xl font-semibold tracking-wide md:text-5xl flex-1 md:w-3/4">Experince that grows with scale.</h1>
        </div>
      ...
      <div className="flex flex-wrap items-center justify-around md:justify-between gap-6 w-full">
        {service.map((item) => (
          <div key={item.id} className="max-w-80 flex flex-col gap-2 max-h-80">
            <item.icon className="text-blue-primary text-6xl"/>
            <h1 className="text-black-primary text-xl font-semibold">{item.title}</h1>
